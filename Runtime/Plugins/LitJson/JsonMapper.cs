...
namespace LitJson
{
    ...
    public class JsonMapper
    {
        ...
        private static void RegisterBaseImporters ()
        {
            ...
            // 遍历所有数据类型组合
            foreach (Type type1 in dataTypes)
            {
                foreach (Type type2 in dataTypes)
                {
                    RegisterImporter(type1, type2);
                }
            }
        }

        private static void RegisterImporter(Type fromType, Type toType)
        {
            RegisterImporter(base_importers_table, fromType, toType, input => Convert.ChangeType(input, toType));
        }
